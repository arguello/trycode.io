...
var pageExitConditions = [
	{ verify: function(data) { return data.expr == "(circle 10)"; }},
	{ verify: function(data) { return data.expr == "(hc-append (circle 10) (rectangle 10 20))"; }},
  { verify: function(data) { return data.expr == "(+ 1 2 3)"; }},
	{ verify: function(data) { return data.expr == "(* 2 (- 5 3))"; }},
	{ verify: function(data) { return data.expr == "(- 5 3)"; }},
...
function setupLink(url) {
    return function(e) { $("#guide").load(url, function(data) { $("#guide").html(data); }); }
}
...
function onHandle(line, report) {
	...
	if (input == 'clear') {
        ...
        error_message = data.message.replace(/context...:[\s\S]*/, '');
				results.push({msg: error_message, className: "jquery-console-message-error"});
			} // handle page
...
function guideUpdated() {
    $("#guide code.expr").each(function() {
        ...
        $(this).attr("title", "Click to insert '" + $(this).text() + "' into the REPL.");
